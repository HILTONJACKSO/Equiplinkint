...
export function EquipmentClient({ heroData }: { heroData: any }) {
    ...
    <>
      ...
      <main className="flex-1 bg-[#F8FAFC]">
        <div className="relative bg-[#132238] border-b border-[#0B1220] py-16 md:py-24 overflow-hidden">
          <div className="absolute inset-0 bg-cover bg-center opacity-10 mix-blend-overlay" style={{ backgroundImage: `url('${heroData.image || "/images/cat_336_excavator.png"}')` }} />
          <div className="absolute inset-0 bg-gradient-to-t from-[#132238] to-transparent" />
          <div className="max-w-7xl mx-auto px-4 relative z-10">
            <h1 className="text-4xl md:text-5xl font-bold font-heading text-white mb-4" dangerouslySetInnerHTML={{ __html: heroData.title || 'Find <span className="text-amber-500">Equipment</span>' }} />
            <p className="text-slate-300 text-lg max-w-2xl">{heroData.subtitle || 'Browse available heavy machinery and logistics services across Liberia. Top-tier brands maintained to the highest standards.'}</p>
          </div>
        </div>
